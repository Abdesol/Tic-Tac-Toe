                ...
                if p[0] != None:
                    ...
                    for i in p:
                        if i != init:
                            ...
                            init_lst.append(i)
                    if len(init_lst) == 3:
                        return [p, all_pos.index(p)+1, p[0]]
            return False
            ...
            if self.is_done != True:
                if self.tic_tac_lst[index[0]][index[1]] == None:
                    ...
                    if result != False:
                        who = result[2]
                        if who == 1:
                            self.turn_label.setText("X player won")
                        else:
                            self.turn_label.setText("O player won")
                        self.is_done = True
                    else:
                        if self.whose_turn == 1:
                            self.turn_label.setText("O players turn")
                            self.whose_turn = 0
                        else:
                            self.turn_label.setText("X players turn")
                            self.whose_turn = 1
